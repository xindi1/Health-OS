const STORAGE_KEY = 'healthosclassic.entries.v1';
const THEME_KEY = 'healthosclassic.theme.v1';
const domains = ['nutrition', 'movement', 'sleep', 'wellness'];
const domainLabels = { nutrition: 'Nutrition', movement: 'Movement', sleep: 'Sleep', wellness: 'Wellness' };
const metricIds = ['weight', 'energy', 'stress', 'mood', 'dailyNote'];
...
function scoreStatus(score) {
  if (score >= 85) return ['Strong health day', 'Your nutrition, movement, sleep, and wellness signals are strong today.'];
  if (score >= 70) return ['Healthy day', 'You are supporting your baseline health well; protect the weakest domain.'];
  if (score >= 50) return ['Mixed signal', 'Choose one small upgrade in food, movement, sleep, or wellness.'];
  if (score > 0) return ['Low signal day', 'Keep it simple: eat real food, move, sleep, hydrate, and lower stress.'];
  return ['Start today’s check-in', 'Complete the four domains to reveal today’s health signal.'];
}
...
function hydrateDate(date) {
  ...
  metricIds.forEach(id => $(id).value = entry?.metrics?.[id] ?? '');
  updateScoreUI();
}
...
function buildEntry() {
  ...
  const metrics = Object.fromEntries(metricIds.map(id => [id, $(id).value.trim()]));
  return { id: crypto.randomUUID(), app: 'HealthOS Classic', date, score: calculateScore(), domains: domainScores, notes, metrics, updatedAt: new Date().toISOString() };
}
...
function resetForm() { domains.forEach(d => { $(d).value = 0; $(`${d}Note`).value = ''; }); metricIds.forEach(id => $(id).value = ''); updateScoreUI(); }
...
function renderHistory() {
  ...
  if (!recent.length) { list.innerHTML = '<p>No entries yet. Save today to start your HealthOS Classic record.</p>'; return; }
...
function exportJSON() { const blob = new Blob([JSON.stringify({ app: 'HealthOS Classic', version: 1, entries }, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `healthos-classic-export-${todayISO()}.json`; a.click(); URL.revokeObjectURL(url); }
...
function init() {
  ...
  $('clearData').addEventListener('click', () => { if (confirm('Clear all HealthOS Classic data on this device?')) { entries = []; saveEntries(); resetForm(); renderAll(); } });
